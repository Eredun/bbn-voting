...
const leaderboardNames = document.querySelectorAll('.username1');
const leaderboardImages = document.querySelectorAll('.leaderboard .username');
...
btn.addEventListener('click', e => {
  if (votesCounter.textContent <= 0) {
    ...
    newContestants.map((user, index, users) => {
        leaderboardNames.forEach((username1, elIndex) => {
          if(index === elIndex) {
        ...
        lowVoteUser = users[users.length - 1].name;
