...
const getContestants = () => {
  ...
  if (!strHousemates) {
    ...
    const housemates = JSON.parse(strHousemates);
    housemates.sort((a, b) => (a.votes > b.votes) ? 1 : (a.votes === b.votes) ? ((a.name > b.name) ? 1 : -1) : -1);
  }
  return housemates;
}
...
btn.addEventListener('click', e => {
  if (votesCounter.textContent <= 0) {
    ...
    // newContestants
    // console.log('Sorted:', newContestants);
  ...
  //    </div>
  // `
  // leaderboard.appendChild(userInfo);
  } else {
